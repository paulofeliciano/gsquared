		...
		<div class="ct-tab-panel ct-dom-tree-tab ct-active" ng-show="isShowTab('sidePanel','DOMTree')">
			...
			<script type="text/ng-template" id="dom-tree-node">
				...
				<div class="sub-tree" ng-class="{'collapsed': state.collapsed}">
					<div draggable="{{isDraggable(item.options.ct_parent)}}" ng-include="'dom-tree-node'" ng-repeat="(key, item) in item.children" ng-init="state={collapsed: true, showMoreOptions: false, showCategorize: false, item: item, editable: false, depth: state.depth+1}"  class="dom-tree-node depth-{{state.depth}}" ng-attr-ng-attr-tree-id="{{item.id}}" new-tree-control="state" ng-class="{'active': iframeScope.component.active.id === item.id}"></div>
				</div>
			</script>
			...
			<div id="ct-dom-tree-2">
				...
				<div class="dom-tree-node depth-1" draggable="true"
					...
					ng-class="{'active': iframeScope.component.active.id === item.id, 'categorized-component': iframeScope.component.options[item.id]['ct_category'] != undefined}">
				</div> 
				...
				<div class="dom-tree-node depth-1" draggable="true"
					...
					ng-attr-ng-attr-tree-id="{{item.id}}" 
					new-tree-control="state" 
					ng-class="{'active': iframeScope.component.active.id === item.id, 'categorized-component': iframeScope.component.options[item.id]['ct_category'] != undefined}"></div> 
			</div>
		</div>
		<!-- .ct-dom-tree-tab -->
